...
function shouldMaskPhone(r: TaesanTask) {
  // DB에서 영구 마스킹된 경우
...
function getDisplayPhone(r: TaesanTask) {
...
// ─── 상태 설정 ────────────────────────────────────────────
const STATUS_ORDER: TaesanStatus[] = ["접수", "신용조회", "확정"];
const CREDIT_STATUSES: TaesanStatus[] = ["승인", "보완", "거절"];

// 신용결과(승인/보완/거절)를 포함한 전체 순서 인덱스
// 신용결과는 신용조회(1) 다음, 서류등록(2) 이전에 위치
const FULL_STATUS_ORDER: TaesanStatus[] = ["접수", "신용조회", "승인", "보완", "거절", "확정"];

function getStatusIndex(status: TaesanStatus): number {
...
function canGoToStatus(current: TaesanStatus, next: TaesanStatus, isAdmin: boolean): boolean {
  ...
  // 일반 순서: 접수 → 신용조회 → 확정
  const mainOrder: TaesanStatus[] = ["접수", "신용조회", "확정"];
...
function statusStyle(status: TaesanStatus) {
  switch (status) {
...
// ─── 서류 목록 ────────────────────────────────────────────
const DOC_FIELDS: { key: keyof TaesanTask; label: string; dbCol: string }[] = [
  ...
  // doc_etc는 taesan_etc_docs 테이블로 분리됨
];
...
export default function TaesanPage() {
  ...
  // ── 데이터 ──
  const [rows,    setRows]    = useState<TaesanTask[]>([]);
  ...
  const [statusFilter,    setStatusFilter]    = useState<TaesanStatus | "all">("all");
  ...
  // ── 수정 모달 ──
  const [editRow,                   setEditRow]                   = useState<TaesanTask | null>(null);
  ...
  const [creditResults, setCreditResults] = useState<Record<string, TaesanStatus>>({});

  // ── 신용결과 상세 입력 모달 ──
  const [creditModal, setCreditModal]           = useState<{ row: TaesanTask; next: TaesanStatus } | null>(null);
  ...
  const KAKAO_RECIPIENTS = [
    ...
    { id: "taesan", label: "태산통운 담당자" },
  ] as const;
  type RecipientId = typeof KAKAO_RECIPIENTS[number]["id"];

  const [holdModal,        setHoldModal]        = useState<TaesanTask | null>(null);
  const [approvalModal,    setApprovalModal]    = useState<TaesanTask | null>(null); // 승인조건 확인 모달
  ...
  const openHoldModal = (row: TaesanTask) => {
    setHoldModal(row);
    // 기본값: 내일 10:00
  ...
  // ── 상환스케줄 PDF 다운로드 ──
  const downloadSchedulePDF = (task: TaesanTask, startYM: string, recipient: string) => {
...
<h1>원리금균등분납 상환스케줄</h1>
<p class="subtitle">RNF Korea · 태산통운 할부금융</p>
...
<p class="footer">※ 실제 납입액은 금융사 기준일·계산방식에 따라 일부 다를 수 있습니다.</p>
  ...
  // ── 확정 승인내역 입력 모달 ──
  const [confirmModal, setConfirmModal]             = useState<TaesanTask | null>(null);
  ...
  const syncHcmToGcal = async (task: {
    ...
    try {
      ...
      const title = `${task.company_name ?? task.customer_name} (${task.equipment_ton ?? "태산통운"}) 접수`;
      const description = [
      ...
      if (d?.task?.id) {
        await supabase.from("taesan_tasks").update({ gcal_task_id: d.task.id }).eq("id", task.id);
      }
    } catch (e) {
  ...
  const completeHcmGcalTask = async (taskId: number | string) => {
    if (!user) return;
    try {
      const { data: row } = await supabase.from("taesan_tasks").select("gcal_task_id").eq("id", taskId).maybeSingle();
  ...
  const uploadEtcDoc = async (rowId: string | number, file: File, docLabel: string) => {
    ...
    try {
      ...
      const path     = `taesan/${String(rowId)}/etc/${safeName}`;
  ...
  const deleteEtcDoc = async (docId: number, storagePath: string, rowId: string | number) => {
    ...
    try {
      await supabase.storage.from("taesan_docs").remove([storagePath]);
      await supabase.from("taesan_etc_docs").delete().eq("id", docId);
      setEtcDocs((prev) => ({
        ...prev,
  ...
  const downloadVehicleRegDoc = async (path: string, name: string) => {
    try {
      ...
      if (isMobile) {
        const { data, error } = await supabase.storage.from("taesan-vehicle-reg-docs").createSignedUrl(path, 60);
        if (error || !data?.signedUrl) throw error ?? new Error("URL 생성 실패");
        window.open(data.signedUrl, "_blank");
      } else {
        const { data, error } = await supabase.storage.from("taesan-vehicle-reg-docs").download(path);
  ...
  const fetchTaxInvoiceFiles = async (rowIds: (string | number)[]) => {
    if (rowIds.length === 0) return;
    const { data } = await supabase
      .from("taesan_tax_invoice_uploads")
  ...
  const downloadTaxInvoice = async (path: string, name: string) => {
    try {
      const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
      if (isMobile) {
        const { data, error } = await supabase.storage.from("taesan-tax-invoices").createSignedUrl(path, 60);
        if (error || !data?.signedUrl) throw error ?? new Error("URL 생성 실패");
        window.open(data.signedUrl, "_blank");
      } else {
        const { data, error } = await supabase.storage.from("taesan-tax-invoices").download(path);
        if (error || !data) { alert("다운로드 실패: " + error?.message); return; }
        const url = URL.createObjectURL(data);
        const a   = document.createElement("a");
        a.href = url; a.download = name;
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      let q = supabase.from("taesan_tasks").select("*");
      if (!isAdmin) {
      ...
      const fetchedRows = (data ?? []) as TaesanTask[];
      // taesan_tasks는 RLS로 admin/subAdmin/taesan 계정만 접근 가능하므로
      // 별도의 클라이언트 필터링이 필요 없음
      const nextRows = fetchedRows;
      setRows(nextRows);
      const drafts: Record<string, string> = {};
      const credits: Record<string, TaesanStatus> = {};
      nextRows.forEach((r) => {
        drafts[String(r.id)] = r.special_note ?? "";
        if (["승인","보완","거절"].includes(r.status)) {
          credits[String(r.id)] = r.status as TaesanStatus;
        }
      });
  ...
  const onAdd = async () => {
    ...
    try {
      ...
      if (inserted) {
        ...
        const regTitle = `${payload.customer_name} (태산통운 - 접수)`;
        const regDesc  = [
          `케이스: ${newCaseNo}`,
  ...
  // ─── 상태 변경 ───────────────────────────────────────────
  const changeStatus = async (row: TaesanTask, next: TaesanStatus) => {
    ...
    const patch: Partial<TaesanTask> = { status: next };
    ...
    const { error } = await supabase.from("taesan_tasks").update(patch).eq("id", row.id as any);
    if (error) {
  ...
  const saveConfirmModal = async () => {
    ...
    try {
      const patch: Partial<TaesanTask> & Record<string, any> = {
        status:               "확정" as TaesanStatus,
  ...
  const saveMemo = async (rowId: string | number) => {
    ...
    try {
      ...
      const { error } = await supabase.from("taesan_tasks").update({ special_note: note || null }).eq("id", rowId as any);
      if (error) throw error;
  ...
  const saveCreditModal = async () => {
    ...
    try {
      const patch: Partial<TaesanTask> = {
        status:              next,
  ...
  // ─── 수정 모달 ───────────────────────────────────────────
  const openEditModal = (row: TaesanTask) => {
  ...
  const saveEditRow = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("taesan_tasks").update(patch).eq("id", editRow.id as any);
      if (error) throw error;
  ...
  const handleDocFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const path = `taesan/${String(rowId)}/${dbCol}.${ext}`;
    setUploadingDocKey(`${rowId}_${dbCol}`);
    try {
      const { error: upErr } = await supabase.storage.from("taesan_docs").upload(path, file, { upsert: true, contentType: file.type || undefined });
      if (upErr) throw upErr;
      const { error: dbErr } = await supabase.from("taesan_tasks").update({ [dbCol]: path }).eq("id", rowId as any);
      if (dbErr) throw dbErr;
  ...
  const downloadDoc = async (path: string, label: string) => {
    try {
      ...
      if (isMobile) {
        ...
        // PC: Blob 다운로드
        const { data, error } = await supabase.storage.from("taesan_docs").download(path);
        if (error) throw error;
        const url = URL.createObjectURL(data);
        const a   = document.createElement("a");
        a.href = url;
  ...
  const deleteRow = async (rowId: string | number) => {
    ...
    try {
      ...
      if (target) {
        ...
        // 기타서류(taesan_etc_docs) storage 경로도 함께 삭제
        const etcPaths = (etcDocs[String(rowId)] ?? []).map((d) => d.path);
        const allPaths = [...paths, ...etcPaths];
        if (allPaths.length > 0) await supabase.storage.from("taesan_docs").remove(allPaths);
      }
      // 삭제 전 gcal_task_id 조회 → 구글 할일도 삭제
      const { data: gcalRow } = await supabase.from("taesan_tasks").select("gcal_task_id").eq("id", rowId as any).maybeSingle();
      const { error } = await supabase.from("taesan_tasks").delete().eq("id", rowId as any);
      if (error) throw error;
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-3.5 shadow-sm">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as TaesanStatus | "all")} className={inputClass}>
                      ...
                      <div className="rounded-xl border border-orange-100 bg-orange-50 px-4 py-3 grid grid-cols-2 gap-2">
                        <div className="col-span-2 flex items-center justify-between mb-1">
                          ...
                          {(() => {
                            ...
                            return (
                              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-xl border text-xs font-bold ${statusStyle(creditStatus as TaesanStatus)}`}>
                                판정: {creditStatus}
                              </span>
                            );
                          })()}
                      ...
                      <div className="flex flex-wrap gap-1.5 pb-1">
                        ...
                        {["접수"].map((s) => {
                          ...
                          <button
                            ...
                            onClick={() => changeStatus(r, s as TaesanStatus)}
                            className={`px-3 py-1 rounded-xl border text-xs font-semibold transition-all
                              ${r.status === s
                                ? statusStyle(s as TaesanStatus) + " ring-2 ring-offset-1 ring-orange-200/60"
          ...
          <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/50 px-4">
            ...
            <div style={{ position: "fixed", left: "-9999px", top: 0, width: "560px" }}>
              <div ref={scheduleTableRef} style={{ fontFamily: "'Malgun Gothic','맑은 고딕',sans-serif", background: "#fff", padding: "24px", color: "#1e293b" }}>
                ...
                <p style={{ fontSize: "12px", color: "#64748b", marginBottom: "16px" }}>태산통운 할부금융</p>
                {scheduleRecipient && (
