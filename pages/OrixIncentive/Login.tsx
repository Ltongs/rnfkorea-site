...
export default function OrixIncentiveLoginPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const { login, user, loading, isOrixAdmin, isOrixPartner } = useAuth() as any;
  ...
  useEffect(() => {
    if (!loading && user && (isOrixAdmin || isOrixPartner)) {
      navigate(redirectTo, { replace: true });
    }
  }, [loading, user, isOrixAdmin, isOrixPartner, navigate, redirectTo]);
