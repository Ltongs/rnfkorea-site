...
export default function RentalOSLoginPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const { login, user, loading, isRentalOS, isAdmin, isSubAdmin } = useAuth() as any;
  ...
  useEffect(() => {
    if (!loading && user && (isRentalOS || isAdmin || isSubAdmin)) {
      navigate(redirectTo, { replace: true });
    }
  }, [loading, user, isRentalOS, isAdmin, isSubAdmin, navigate, redirectTo]);
