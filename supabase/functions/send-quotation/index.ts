...
// 견적서·발주서 이메일 발송
// - 수신 2개(toList) + 참조 2개(ccList) 지원
// - htmlBody를 직접 받아서 이메일 본문으로 사용
//
...
serve(async (req) => {
  ...
  try {
    ...
    const typeLabel: Record<string, string> = {
      battery:"배터리 견적서", forklift:"지게차 견적서",
      installment:"할부 견적서", purchase:"발주서",
    };
    ...
    if (quoteType === "installment" && installmentInfo) {
      ...
      emailHtml = `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"></head>
...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;">
  <tr><td style="background:#0a192f;padding:24px 32px;">
    <table width="100%"><tr>
      ...
      <td style="text-align:right;"><div style="color:#f97316;font-size:18px;font-weight:bold;">할부 견적서</div>
      <div style="color:#94a3b8;font-size:10px">No. ${quoteNo}</div></td>
    </tr></table>
  </td></tr>
  <tr><td style="padding:24px 32px;">
    <p style="font-size:14px;color:#1e293b;margin:0 0 16px;"><strong>${recipient}</strong> 귀중,</p>
    <p style="font-size:13px;color:#374151;margin:0 0 20px;">안녕하세요. 주식회사 알앤에프코리아입니다.<br>요청하신 할부 견적서를 안내드립니다.</p>
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#f1f5f9;border-radius:6px;padding:16px 20px;margin-bottom:16px;">
      ...
      <tr><td style="font-size:13px;font-weight:bold;color:#0a192f;padding:4px 0">월 납입액</td>
          <td style="font-size:18px;font-weight:bold;color:#f97316;text-align:right">${fmt(ii.payment)}원</td></tr>
    </table>
    ${extraMessage ? `<p style="font-size:12px;color:#374151;white-space:pre-wrap;margin:0 0 16px;">${extraMessage}</p>` : ""}
    <p style="font-size:11px;color:#64748b;margin:0">※ 실제 납입액은 금융사 기준일·계산방식에 따라 일부 다를 수 있습니다.</p>
  </td></tr>
    ...
    <div style="color:#94a3b8;font-size:10px;margin-top:4px">TEL: 1551-1873 | rnfkorea.co.kr</div>
      ...
      emailHtml = `<html><body style="font-family:Arial;max-width:600px;margin:auto;padding:20px;">
        ...
        <table style="border-collapse:collapse;width:100%;margin:16px 0">
          ...
          <tr><td style="padding:8px;background:#0a192f;color:#fff;font-weight:bold">총액</td>
              <td style="padding:8px;text-align:right;background:#0a192f;color:#f97316;font-weight:bold;font-size:16px">${fmt(grandTotal)}원</td></tr>
        </table>
